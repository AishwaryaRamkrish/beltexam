...
module.exports = {
  ...
  index: function (req, res) {
    Primary.find({}, function (err, data) {
      if (err) {
        res.json({ errorMsg: "Couldn't find properly", error: err })
      } else {
  ...
  addNew: function(req, res){
    ...
    primary.save(function(err, data){
      if (err) {
        res.json({ errorMsg: "Couldn't create properly", error: err })
      } else {
  ...
  addSecondary: function(req, res){
    Primary.updateOne({_id:req.params.id}, {$push:{reviews:req.body}}, function(err, data){
      if(err){
        res.json({errorMsg:"Could not update the object with the nested object", error: err})
      }else {
  ...
  destroy:function(req, res){
    Primary.remove({_id:req.params.id}, function(err,data){
      if(err){
        res.json({errorMsg:"Could not delete the object", error: err})
      }else {
